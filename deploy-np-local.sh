...
printf '%s' "Get Cloud One Smart Check node port port"

DSSC_PORT=''
while [ "$DSSC_PORT" == '' ]
do
  DSSC_PORT=$(kubectl get svc -n smartcheck proxy \
...
then
  ...
  do
    DSSC_USERID=`curl -s -k -X POST https://${DSSC_HOST}:${DSSC_PORT}/api/sessions \
                    -H "Content-Type: application/json" \
                    -H "Api-Version: 2018-05-01" \
                    -H "cache-control: no-cache" \
                    -d "{\"user\":{\"userid\":\"${DSSC_USERNAME}\",\"password\":\"${DSSC_TEMPPW}\"}}" | \
                      jq '.user.id' | tr -d '"'  2>/dev/null`
    DSSC_BEARERTOKEN=`curl -s -k -X POST https://${DSSC_HOST}:${DSSC_PORT}/api/sessions \
                    -H "Content-Type: application/json" \
                    -H "Api-Version: 2018-05-01" \
                    -H "cache-control: no-cache" \
                    -d "{\"user\":{\"userid\":\"${DSSC_USERNAME}\",\"password\":\"${DSSC_TEMPPW}\"}}" | \
  ...
  DUMMY=`curl -s -k -X POST https://${DSSC_HOST}:${DSSC_PORT}/api/users/${DSSC_USERID}/password \
          -H "Content-Type: application/json" \
          -H "Api-Version: 2018-05-01" \
          -H "cache-control: no-cache" \
...
[san]
  subjectAltName=DNS:${DSSC_HOST}
EOF

openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 -nodes -keyout k8s.key -out k8s.crt -subj "/CN=*${DSSC_HOST}" -extensions san -config req.conf
kubectl create secret tls k8s-certificate --cert=k8s.crt --key=k8s.key --dry-run=client -n ${DSSC_NAMESPACE} -o yaml | kubectl apply -f - > /dev/null
